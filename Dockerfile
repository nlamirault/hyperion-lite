...
ENV STATSD_VERSION 0.7.1
ENV GRAFANA_VERSION 1.7.0
ENV LOGSTASH_VERSION 1.4.2
ENV KIBANA_VERSION 1.3.1
...
RUN mkdir -p /src/elasticsearch && cd /src/elasticsearch && \
    ...
    tar xzf elasticsearch-$ES_VERSION.tar.gz --strip-components=1 && rm elasticsearch-$ES_VERSION.tar.gz
...
RUN mkdir -p /src/statsd && cd /src/statsd && \
    wget https://github.com/etsy/statsd/archive/v$STATSD_VERSION.tar.gz && \
    tar xzvf v$STATSD_VERSION.tar.gz --strip-components=1 && rm v$STATSD_VERSION.tar.gz
...
RUN mkdir -p /src/logstash && cd /src/logstash && \
    wget -q https://download.elasticsearch.org/logstash/logstash/logstash-$LOGSTASH_VERSION.tar.gz && \
    tar xzf logstash-$LOGSTASH_VERSION.tar.gz --strip-components=1 && \
...
ADD ./grafana/grafana_config.js /src/grafana/config.js
ADD ./grafana/hyperion-lite.json /src/grafana/app/dashboards/default.json
